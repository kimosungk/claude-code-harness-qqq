...
- The qqq dispatcher pre-attaches `phase1-spec.md`, `phase1-tech-spec.md` (required), `phase1-ui-outline.md` (if present on disk), and `phase1-nltp.md` (if present on disk) as `@`-mentions in your first user turn. Use that attached content directly — do NOT call `Read` on those files again. When consuming `phase1-tech-spec.md`, **use only the content up to the `<!-- audit-only-below — readers must stop here -->` anchor line** — content past that anchor (§10 Decision Audit Trail) is autonomy-tier governance metadata and is NOT plan input.
- Do not Read or list sidecar artifacts in the session dir (`phase1-tech-spec-scope-lint.md`, `phase1-tech-spec-sanity*.md`, `phase1-tech-spec-sanity-output.json`). They are intentionally not in your attachments — only the four primary inputs above feed planning.
